...
// Add to Cart
func (r *mutationResolver) AddToCart(
	ctx context.Context,
	input model.AddToCartInput,
) (*model.AddToCartResponse, error) {
...
func (r *mutationResolver) RemoveFromCart(ctx context.Context, variatnID string) (*model.Response, error) {
	userID, ok := utils.GetUserIDFromContext(ctx)
	if !ok {
		return &model.Response{
			Success: false,
